...
export function activate(context: vscode.ExtensionContext) {
  ...
  const extensionDisplayName = String(
    context.extension.packageJSON.displayName ?? context.extension.packageJSON.name ?? "New Db Viewer"
  );
  const provider = new DbViewerCustomEditorProvider(
...
async function pickDatabaseFile(): Promise<vscode.Uri | undefined> {
...
class DbViewerDocument implements vscode.CustomDocument {
...
class DbViewerCustomEditorProvider implements vscode.CustomReadonlyEditorProvider<DbViewerDocument> {
  constructor(
    ...
    webviewPanel.webview.onDidReceiveMessage((message) => {
      ...
      if (message?.type === "db:save") {
        ...
        this.output.appendLine(
          `db:save received bytes=${bytes ? bytes.byteLength : "invalid"}`
        );
        if (!bytes) {
...
async function openWithViewer(uri: vscode.Uri, output: vscode.OutputChannel) {
...
const openGate = new Set<string>();

function isDatabaseFile(uri: vscode.Uri): boolean {
...
function normalizeMessageBytes(value: unknown): Uint8Array | undefined {
